...
for (int loop=9; loop<10; loop++){
    ...
    //THIS IS ADDED on recommendation from Feng to add penalty term to keep the Ug from getting too large.
	
	auto&& target_function = [&abs_difference_function, penalty_weight, ug_numbers](double* ugs)
	{
		return penalty_weight * std::inner_product(ugs, ugs + ug_numbers, ugs, 0.0) + abs_difference_function(ugs);
	};
