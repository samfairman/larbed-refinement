...
for (int scalingFactor=1; scalingFactor<(3); scalingFactor=(scalingFactor*10+1)){
for (int loop=0; loop<1; loop++){
    ...
    double randMax = 0.001*scalingFactor;
	...
	//ug_initial[2*i] = ug_initial[2*i]-r;
        //ug_initial[2*i+1] = ug_initial[2*i+1] -r;
}  
    double r = randMin + static_cast <double> (rand()) /( static_cast <double> (RAND_MAX/(randMax-randMin)));
    ug_initial[0]=1.0;
    ug_initial[1]=0.0;
	...
	if( i==0 ){ std::cout<<"Initial Ugs:"<<std::endl; }	
    std::cout <<"\n"<< i << "    "<< ug_initial[i] << "   " << ug_initial[i+1] << std::endl; 
    }
     
    auto const& merit_function = cpt.make_merit_function();
    int penalty_weight= 10;
    int tilt_size = pt.tilt_size;
   
//Feng to add penalty term to keep the Ug from getting too large.
 	
	//auto&& target_function = [&merit_function, penalty_weight, ugSize, tilt_size](double* ugs)
	auto&& target_function = [&merit_function, tilt_size](double* ugs)
	{
		//return penalty_weight * std::inner_product(ugs, ugs + ugSize, ugs, 0.0) + (merit_function(ugs) /tilt_size ) ;
		return merit_function(ugs) / (tilt_size);
	};

// Sam code ends

    //f::simple_steepest_descent<double> sd( target_function, pt.ug_size * 2 + 1 );
    ...
    //stream << std::fixed <<std::setprecision(1) << thickness_;
    //std::string tk = stream.str();
    ...
    on_iteration_over( target_function(ug_initial.data()),ug_initial.data() );
